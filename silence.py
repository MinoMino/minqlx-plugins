...
class silence(minqlx.Plugin):
    ...
    def cmd_silence(self, player, msg, channel):
        ...
        if self.db.has_permission(ident, 2):
            channel.reply("^6{}^7 has permission level 5 and cannot be silenced.".format(name))
        #    return
    ...
    def cmd_checksilence(self, player, msg, channel):
        ...
        if res:
            ...
            if reason:
                channel.reply("^6{}^7 is silenced until ^6{}^7 for the following reason:^6 {}".format(name, *res))
            else:
